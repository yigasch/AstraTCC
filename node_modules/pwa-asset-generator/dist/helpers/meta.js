...
const generateOutputPath = (options, imageName, imagePath, isManifest = false) => {
    ...
    if (pathOverride) {
        return `${pathOverride}/${imageName}${path_1.default.extname(imagePath)}`;
    }
...
const generateAppleTouchIconHtml = (savedImages, options) => {
    return savedImages
        ...
        .map(({ path: imagePath, name }) => constants_1.default.APPLE_TOUCH_ICON_META_HTML(generateOutputPath(options, name, imagePath), options.xhtml))
        .join('');
};
const generateFaviconHtml = (savedImages, options) => {
    return savedImages
        ...
        .map(({ width, path: imagePath, name }) => constants_1.default.FAVICON_META_HTML(width, generateOutputPath(options, name, imagePath), mime_types_1.lookup(imagePath), options.xhtml))
        .join('');
};
const generateMsTileImageHtml = (savedImages, options) => {
    return savedImages
        ...
        .map(({ width, height, path: imagePath, name }) => constants_1.default.MSTILE_IMAGE_META_HTML(constants_1.default.MSTILE_SIZE_ELEMENT_NAME_MAP[`${width}x${height}`], generateOutputPath(options, name, imagePath), options.xhtml))
        .join('');
};
const generateAppleLaunchImageHtml = (savedImages, options, darkMode) => {
    return savedImages
        ...
        .map(({ width, height, path: imagePath, name, scaleFactor, orientation }) => constants_1.default.APPLE_LAUNCH_SCREEN_META_HTML(width, height, generateOutputPath(options, name, imagePath), scaleFactor, orientation, darkMode, options.xhtml))
        .join('');
};
...
const addMetaTagsToIndexPage = (htmlMeta, indexHtmlFilePath, xhtml) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return file_1.default.writeFile(indexHtmlFilePath, pretty_1.default($.html(), { ocd: true }));
});
exports.default = {
    formatMetaTags,
    addIconsToManifest,
